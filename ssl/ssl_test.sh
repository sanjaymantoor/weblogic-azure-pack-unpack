...
# CN name vary depending on the way certificate is created
openssl x509 -noout -subject -in server.crt | sed 's/^[[:blank:]]*//; s/[[:blank:]]*$//; s/[[:blank:]]\{1,\}//g' | sed 's/ //g' | grep -i "CN=${SSL_CERT_CN_NAME}" > /dev/null 2>&1
